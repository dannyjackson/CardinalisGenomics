...
# install pcangsd
git clone https://github.com/Rosemeis/pcangsd
cd pcangsd/

python3 setup.py build_ext --inplace

cd ~/programs/pcangsd/pcangsd/
...
~/programs/angsd/angsd -vcf-gl genolike.bcf -doPlink 2 -out genolike_plink -doGeno -1 -dopost 1 -domaf 1 -doMajorMinor 1 -sites /xdisk/mcnew/dannyjackson/cardinals/referencelists/sites_headless.mafs


plink --tped /xdisk/mcnew/dannyjackson/cardinals/vcf_likelihoods/all/genolike_plink.tped --tfam /xdisk/mcnew/dannyjackson/cardinals/vcf_likelihoods/all/genolike_plink.tfam --allow-extra-chr --snps-only 'just-acgt' --indep-pairwise 50kb 1 0.5 --out genolike_filtered

plink --tped /xdisk/mcnew/dannyjackson/cardinals/vcf_likelihoods/all/genolike_plink.tped --tfam /xdisk/mcnew/dannyjackson/cardinals/vcf_likelihoods/all/genolike_plink.tfam --allow-extra-chr --snps-only 'just-acgt' --extract genolike_filtered.prune.in --out genolike_pruned --make-bed 

bgzip genolike_pruned.beagle.dat

# 2090305 of 2591209
# 500,904 unlinked variants remain


# for selection / this is only able to look at differences between species not urban/rural

pcangsd -b /xdisk/mcnew/dannyjackson/cardinals/vcf_likelihoods/all/genolike.beagle.gz -o /xdisk/mcnew/dannyjackson/cardinals/pca/all -t 12 -e 2 --selection --pcadapt --sites_save --snp_weights

# for pop gen
pcangsd -p /xdisk/mcnew/dannyjackson/cardinals/vcf_likelihoods/all/genolike_pruned -o /xdisk/mcnew/dannyjackson/cardinals/pca/pruned -t 12 -e 2 --selection --admix

sbatch pcangsd.sh 
Submitted batch job 3353730


# it's possible that UWBM77548 and UWBM77718 are related...

# save the following as: /xdisk/mcnew/dannyjackson/cardinals/referencelists/sample_info.txt
...
# plot it in R
cd /xdisk/mcnew/dannyjackson/cardinals/pca/
...
tab <- read.table("/xdisk/mcnew/dannyjackson/cardinals/referencelists/sample_info.txt", header = TRUE)
labs <- data.frame(tab)
# Plot PCA plot
e <- eigen(C)
...
zcat /xdisk/mcnew/dannyjackson/cardinals/vcf_likelihoods/all/genolike.beagle.gz | awk '{print $1}' > snps.txt

tail -n +2 snps.txt > snps_headless.txt

sed -i 's/\.1//g' snps_headless.txt
sed -i 's/\_/\,/g' snps_headless.txt

paste -d ' ' snps_headless.txt all.sites| awk '$2>0 {print $1}' > snps_filtered.txt



paste -d ',' snps_filtered.txt all.pcadapt.test.txt all.pcadapt.pval.txt all.pcadapt.padj.txt > pcadapt_stats_matrix.csv

sed -i 's/VYXE//g' pcadapt_stats_matrix.csv
...
library(qqman)
stats<-read.csv('pcadapt_stats_matrix.csv', header=FALSE)
SNP<-c(1: (nrow(stats)))
...
pdf(file = "pcadapt_pval.pdf", width = 20, height = 7, useDingbats=FALSE)
print(manhattan(mydf,chr="CHROM",bp="POS",p="PVAL",snp="SNP",logp=FALSE,ylab="pcadapt selection scan"))
dev.off()


pdf(file = "pcadapt_padj.pdf", width = 20, height = 7, useDingbats=FALSE)
print(manhattan(mydf,chr="CHROM",bp="POS",p="PADJ",snp="SNP",logp=FALSE,ylab="pcadapt selection scan"))
dev.off()




sbatch pcadapt_manhattan.sh
Submitted batch job 11096757

# useless plot

awk -F ',' '$5<0.01 {print "VYXE"$1".1", $2}' pcadapt_stats_matrix.csv > pcadapt_significantsnps.txt


/xdisk/mcnew/dannyjackson/cardinals/reference_dataset/ncbi_dataset/data/GCA_013397215.1/GCA_013397215.1_ASM1339721v1_genomic.fna
...
grep "$chr" /xdisk/mcnew/dannyjackson/cardinals/reference_dataset/ncbi_dataset/data/GCA_013397215.1/genomic.gff | grep 'ID=gene' | awk '{OFS = "\t"} ($4 < '$midpos' && $5 > '$midpos')' >> relevantgenes_pcadapt.txt

done < pcadapt_significantsnps.txt


awk '{OFS = "\t"} {split($9, arr, ";"); print(arr[2])}' relevantgenes_pcadapt.txt | sed 's/Name\=//g' | sort -u > relevantgenenames_pcadapt.txt
